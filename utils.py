...
def create_line_coordinates(cell_szie: int) -> list[list[tuple]]:
    ...
    for y in range(1, 9):
        # horizontal
        temp = []
        # x, y points [(0, 100), (0, 200),....]
        temp.append((0, y * cell_szie))
        # x, y points [(900, 100), (900, 200)...]
        temp.append((Length_table, y * cell_szie))
        points.append(temp)
    ...
    for x in range(1, 10):
        ...
        temp.append((x * cell_szie, Length_table))
        points.append(temp)
    return points
...
def create_grid(sub_grid: int) -> list[list]:
    ...
    rows = [
        g * sub_grid + r
        for g in shuffle(row_base)
        for r in shuffle(row_base)
    ]
    cols = [
        g * sub_grid + c
        for g in shuffle(row_base)
        for c in shuffle(row_base)
    ]
