class SelectNumber:
    ...
    def draw_button(self, pygame, surface):
        for index, pos in enumerate(self.btn_positions):
            if self.selected_number - 1 == index:
                pygame.draw.rect(surface, self.color_selected, [pos[0], pos[1],
                                 self.btn_w, self.btn_h], width=3,
                                 border_radius=10)
                text_surface = self.my_font.render(str(index + 1),
                                                   False, self.color_selected)
            else:
            ...
            # checking for mouse hover
                if self.button_hover(pos):
                    ...
                    text_surface = self.my_font.render(str(index + 1), False,
                                                       self.color_selected)
                else:
                    text_surface = self.my_font.render(str(index + 1), False,
                                                       self.color_normal)
        ...
        pygame.draw.rect(surface, self.color_normal,
                         [*self.hint_button_pos, self.btn_w, self.btn_h],
                         width=3, border_radius=10)
        hint_text = self.my_font.render('Hint', False, self.color_normal)
        surface.blit(hint_text, (self.hint_button_pos[0] + 7,
                                 self.hint_button_pos[1] + 20))
    ...
    def is_hint_button_clicked(self, mouse_x, mouse_y):
        return self.hint_button_pos[0] < mouse_x and \
            mouse_x < self.hint_button_pos[0] + self.btn_w and \
            self.hint_button_pos[1] < mouse_y and \
            mouse_y < self.hint_button_pos[1] + self.btn_h
